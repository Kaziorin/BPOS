...
const GROUPS: Record<string, { label: string; tone: string }> = {
  OPERATING: { label: "Operating & Overhead", tone: "bg-brand-50 text-brand-dark border-brand-border" },
  ...
  MISC: { label: "Miscellaneous", tone: "bg-slate-100 text-gray-600 border-slate-200" },
};
            ...
            columns={[
              {
                ...
                render: (c: Category) => (
                  <div>
                    <p className="font-bold text-gray-600 text-xs">{c.name}</p>
                    {c.description && (
              ...
              },
              {
                ...
                render: (c: Category) => {
                  ...
                  return (
                    <span
                      className={`inline-flex rounded-sm px-2 py-0.5 text-[11px] font-semibold border ${g.tone}`}
                    >
                      {g.label}
                    </span>
                  );
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 pt-1">
            {filteredCategories.map((c) => {
              ...
              return (
                <div
                  key={c.id}
                  className="rounded-sm border border-slate-200 bg-white p-3.5 shadow-2xs hover:border-brand-border transition space-y-2 flex flex-col justify-between"
                >
                  <div className="space-y-1.5">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold text-xs text-gray-600">{c.name}</h3>
                      </div>
                      <span className={`rounded-sm px-1.5 py-0.2 text-[10px] font-semibold border ${g.tone}`}>
                        {g.label.split(" ")[0]}
                      </span>
                    </div>
                    {c.description && (
                  ...
                  <div className="border-t border-slate-100 pt-2 flex items-center justify-between text-xs">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="p-1 text-slate-400 hover:text-brand-primary rounded-sm"
                      >
                        <Edit3 size={13} />
                      </button>
                      <button
                        onClick={() => setDeleteConfirm({ id: c.id, name: c.name })}
                        className="p-1 text-slate-400 hover:text-red-600 rounded-sm"
                      >
                        <Trash2 size={13} />
                      </button>
                    </div>
