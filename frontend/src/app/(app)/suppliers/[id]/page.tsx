    ...
    <div className="w-full px-4 sm:px-8 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          ...
          <div key={stat.label} className="rounded-sm border border-gray-200 bg-white p-4">
            <p className="text-xs text-gray-500">{stat.label}</p>
            <p className="mt-1 text-xl font-bold text-gray-600">{stat.value}</p>
          </div>
        ))}
      </div>
        ...
        <div className="flex gap-6">
          {tabs.map((tab) => (
            <button
              ...
              className={`border-b-2 pb-3 text-sm font-medium transition ${activeTab === tab.key ? "border-brand-primary text-primary-600" : "border-transparent text-gray-500 hover:text-gray-600"}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        <div className="rounded-sm border border-gray-200 bg-white p-6 space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              {editing ? <input type="text" value={form.name || ""} onChange={(e) => setForm({ ...form, name: e.target.value })} className={inputClass} /> : <p className="mt-1 text-sm text-gray-600">{supplier.name}</p>}
            </div>
            <div>
              <label className={labelClass}>Company</label>
              {editing ? <input type="text" value={form.company || ""} onChange={(e) => setForm({ ...form, company: e.target.value })} className={inputClass} /> : <p className="mt-1 text-sm text-gray-600">{supplier.company || "—"}</p>}
            </div>
            <div>
              <label className={labelClass}>Contact Person</label>
              {editing ? <input type="text" value={form.contactPerson || ""} onChange={(e) => setForm({ ...form, contactPerson: e.target.value })} className={inputClass} /> : <p className="mt-1 text-sm text-gray-600">{supplier.contactPerson || "—"}</p>}
            </div>
            <div>
              <label className={labelClass}>Phone</label>
              {editing ? <input type="text" value={form.phone || ""} onChange={(e) => setForm({ ...form, phone: e.target.value })} className={inputClass} /> : <p className="mt-1 text-sm text-gray-600">{supplier.phone || "—"}</p>}
            </div>
            <div>
              <label className={labelClass}>Email</label>
              {editing ? <input type="email" value={form.email || ""} onChange={(e) => setForm({ ...form, email: e.target.value })} className={inputClass} /> : <p className="mt-1 text-sm text-gray-600">{supplier.email || "—"}</p>}
            </div>
            <div>
              <label className={labelClass}>City</label>
              {editing ? <input type="text" value={form.city || ""} onChange={(e) => setForm({ ...form, city: e.target.value })} className={inputClass} /> : <p className="mt-1 text-sm text-gray-600">{supplier.city || "—"}</p>}
            </div>
            <div className="sm:col-span-2">
              <label className={labelClass}>Address</label>
              {editing ? <input type="text" value={form.address || ""} onChange={(e) => setForm({ ...form, address: e.target.value })} className={inputClass} /> : <p className="mt-1 text-sm text-gray-600">{supplier.address || "—"}</p>}
            </div>
            <div>
              <label className={labelClass}>VAT Reg No</label>
              {editing ? <input type="text" value={form.vatRegNo || ""} onChange={(e) => setForm({ ...form, vatRegNo: e.target.value })} className={inputClass} /> : <p className="mt-1 text-sm text-gray-600">{supplier.vatRegNo || "—"}</p>}
            </div>
            <div>
              <label className={labelClass}>Payment Terms (days)</label>
              {editing ? <input type="number" value={form.paymentTermsDays || ""} onChange={(e) => setForm({ ...form, paymentTermsDays: Number(e.target.value) })} className={inputClass} /> : <p className="mt-1 text-sm text-gray-600">{supplier.paymentTermsDays ?? "—"}</p>}
            </div>
            <div>
              <label className={labelClass}>Credit Limit</label>
              {editing ? <input type="number" value={form.creditLimit || ""} onChange={(e) => setForm({ ...form, creditLimit: Number(e.target.value) })} className={inputClass} /> : <p className="mt-1 text-sm text-gray-600">৳{Number(supplier.creditLimit).toFixed(0)}</p>}
            </div>
            <div>
              <label className={labelClass}>Current Due</label>
              <p className="mt-1 text-sm text-gray-600">৳{Number(supplier.currentDue).toFixed(0)}</p>
            </div>
            <div>
              <label className={labelClass}>Rebate %</label>
              {editing ? <input type="number" value={form.rebatePercent || ""} onChange={(e) => setForm({ ...form, rebatePercent: Number(e.target.value) })} className={inputClass} /> : <p className="mt-1 text-sm text-gray-600">{supplier.rebatePercent ?? "—"}%</p>}
            </div>
          </div>
          {supplier.notes && (
            <div>
        ...
        <div className="rounded-sm border border-gray-200 bg-white p-6 space-y-6">
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            {[
              ...
              { label: "Total POs", value: String(supplier.stats?.totalPOs || 0), color: "text-gray-600" },
            ].map((stat) => (
              <div key={stat.label} className="rounded-sm border border-gray-200 p-4">
                <p className="text-xs text-gray-500">{stat.label}</p>
              ...
              <tbody className="divide-y divide-gray-100">
                {supplier.recentPOs.map((po: any) => (
                  <tr key={po.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 font-medium text-gray-600">{po.poNo}</td>
                    <td className="px-4 py-3 text-right text-gray-600">৳{Number(po.total).toFixed(0)}</td>
