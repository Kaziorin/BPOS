...
const STATUS_CONFIG: Record<string, { label: string; bg: string; text: string; border: string }> = {
  ...
  CONVERTED: { label: "Converted to Order", bg: "bg-brand-50", text: "text-brand-dark", border: "border-brand-border" },
    ...
    <div className="space-y-5 w-full px-4 sm:px-8 pb-12">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5">
        ...
        <div className="rounded-sm border border-gray-200 bg-white p-4 shadow-xs hover:border-gray-300 transition">
          ...
          <div className="mt-2.5 flex items-baseline gap-2">
            ...
            <span className="text-xs font-medium text-brand-primary">win rate</span>
          </div>
          <p className="mt-1 text-[11px] text-gray-400">
      ...
      {loading ? (
        <div className="rounded-sm border border-gray-200 bg-white p-12 text-center shadow-xs">
          ...
          <p className="text-xs font-semibold text-gray-600">Loading quotations...</p>
        </div>
      ) : paginatedQuotations.length === 0 ? (
        <div className="rounded-sm border-2 border-dashed border-gray-200 bg-white p-12 text-center shadow-xs">
          <FileText size={32} className="mx-auto text-gray-300 mb-3" />
          <h3 className="text-sm font-bold text-gray-600">No Quotations Found</h3>
                  ...
                  return (
                    <tr key={q.id} className="hover:bg-gray-50/70 transition-colors">
                      ...
                      <td className="px-3 py-3">
                        <div>
                          <p className="font-semibold text-gray-600">{q.customer?.name || "Prospective Client"}</p>
                          {q.customer?.phone && (
                        ...
                        <div className="flex items-center justify-end gap-1.5 flex-wrap">
                          {q.status === "DRAFT" && (
                            <button
                              ...
                              className="inline-flex items-center gap-1 rounded-sm bg-blue-50 text-blue-700 border border-blue-200 px-2 py-1 text-[11px] font-semibold hover:bg-blue-100 transition"
                          ...
                          {(q.status === "SENT" || q.status === "CUSTOMER_REVIEW") && (
                            <>
                              <button
                                ...
                                className="inline-flex items-center gap-1 rounded-sm bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-1 text-[11px] font-semibold hover:bg-emerald-100 transition"
                                title="Mark Accepted"
                              >
                              ...
                              <button
                                ...
                                className="inline-flex items-center gap-1 rounded-sm bg-rose-50 text-rose-700 border border-rose-200 px-2 py-1 text-[11px] font-semibold hover:bg-rose-100 transition"
                                title="Mark Rejected"
                              >
                          ...
                          {q.status === "ACCEPTED" && (
                            <button
                              ...
                              className="inline-flex items-center gap-1 rounded-sm bg-brand-primary text-white px-2.5 py-1 text-[11px] font-semibold shadow-2xs hover:bg-brand-dark transition"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-3 pt-2.5 border-t border-gray-100">
                    <div className="rounded-sm bg-gray-50 p-2 text-center">
                      ...
                      <span className="text-xs font-bold text-gray-600">৳{Number(q.total || 0).toLocaleString()}</span>
                    </div>
                    <div className="rounded-sm bg-gray-50 p-2 text-center">
                ...
                <div className="mt-3 pt-2.5 border-t border-gray-100 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    {q.status === "ACCEPTED" ? (
                      <button
                        onClick={() => doAction(q.id, "convert")}
                        className="rounded-sm bg-brand-primary text-white px-2 py-0.5 text-[11px] font-semibold hover:bg-brand-dark"
                      >
                        Convert to Order
                      </button>
                    ) : (
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 rounded-sm border border-gray-200 bg-white p-3 shadow-xs">
          <p className="text-xs text-gray-500">
            ...
            of <span className="font-semibold text-gray-600">{totalItems}</span> quotations
          </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 backdrop-blur-xs p-4 overflow-y-auto animate-in fade-in duration-100">
          <div className="relative w-full max-w-3xl rounded-sm bg-white shadow-2xl border border-gray-200 overflow-hidden my-8 animate-in zoom-in-95 duration-150">
            ...
            <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4 bg-gray-50/80">
              <div className="flex items-center gap-2.5">
                <div className="flex h-9 w-9 items-center justify-center rounded-sm bg-primary-50 text-primary-600 border border-primary-200/60 font-bold">
                  <FileSpreadsheet size={18} />
                </div>
                <div>
                  <h2 className="text-sm font-bold text-gray-600">Create Sales Quotation / Estimate</h2>
                  <p className="text-[11px] text-gray-500">Draft customized pricing, discounts, and itemized proforma estimates.</p>
                </div>
              </div>
              <button
                onClick={() => setShowCreateModal(false)}
                className="rounded-sm p-1.5 text-gray-400 hover:bg-gray-200/60 transition"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <form onSubmit={createQuotation} className="p-6 max-h-[75vh] overflow-y-auto space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-[11px] font-semibold text-gray-600 mb-1">Customer / Client Name</label>
                  <input
                    type="text"
                    placeholder="e.g. Apex Corporation"
                    value={form.customerName}
                    onChange={(e) => setForm({ ...form, customerName: e.target.value })}
                    className="w-full rounded-sm border border-gray-300 px-3 py-1.5 text-xs text-gray-600 focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
                  />
                </div>
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-gray-600 mb-1">Valid Until Date</label>
                  <input
                    type="date"
              ...
              <div className="space-y-2 pt-2 border-t border-gray-100">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold text-gray-600 uppercase tracking-wider">Itemized Line Products</h3>
                  <button
                    type="button"
                    onClick={addItem}
                    className="inline-flex items-center gap-1 rounded-sm border border-primary-200 bg-primary-50 px-2.5 py-1 text-xs font-semibold text-primary-700 hover:bg-primary-100 transition"
                  >
                ...
                <div className="space-y-2">
                  {form.items.map((item, idx) => {
                    ...
                    return (
                      <div key={idx} className="grid grid-cols-12 gap-2 p-2.5 rounded-sm border border-gray-200 bg-gray-50/50 items-center text-xs">
                        <div className="col-span-5">
                          ...
                          <input
                            ...
                            className="w-full rounded-sm border border-gray-300 bg-white px-2.5 py-1 text-xs text-gray-600 focus:outline-none focus:border-primary-500"
                          />
                        </div>
                        ...
                        <div className="col-span-2">
                          ...
                          <input
                            ...
                            className="w-full rounded-sm border border-gray-300 bg-white px-2 py-1 text-xs text-gray-600 focus:outline-none focus:border-primary-500"
                          />
                        </div>
                        ...
                        <div className="col-span-2">
                          ...
                          <span className="block font-bold text-gray-600 py-1">৳{lineTotal.toLocaleString()}</span>
                        </div>
                        ...
                        <div className="col-span-1 text-right pt-4">
                          <button
                            ...
                            className="rounded-sm p-1 text-gray-400 hover:text-rose-600 disabled:opacity-30 transition"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
              ...
              {/* Summary Totals Card */}
              <div className="rounded-sm border border-gray-200 bg-gray-50/70 p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
                <div className="flex-1 w-full">
                  <label className="block text-[11px] font-semibold text-gray-600 mb-1">Notes & Terms</label>
                  <input
                    type="text"
                    placeholder="Payment terms, delivery schedules, warranty conditions..."
                    value={form.note}
                    onChange={(e) => setForm({ ...form, note: e.target.value })}
                    className="w-full rounded-sm border border-gray-300 bg-white px-3 py-1.5 text-xs text-gray-600 focus:outline-none focus:border-primary-500"
                  />
                </div>
                ...
                <div className="w-full sm:w-56 text-right space-y-1 text-xs">
                  <div className="flex justify-between text-gray-600">
                    <span>Subtotal:</span>
                    <span className="font-semibold text-gray-600">৳{formSubtotal.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-gray-600 font-bold text-sm pt-1 border-t border-gray-200">
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-gray-100">
                <button
                  type="button"
                  onClick={() => setShowCreateModal(false)}
                  className="rounded-sm border border-gray-300 px-3 py-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-50 transition"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="inline-flex items-center gap-1.5 rounded-sm bg-primary-600 px-4 py-1.5 text-xs font-semibold text-white shadow-sm hover:opacity-90 transition"
                >
                  Create Quotation
                </button>
              </div>
            ...
            <div className="w-screen max-w-md bg-white shadow-2xl border-l border-gray-200 flex flex-col animate-in slide-in-from-right duration-150">
              ...
              <div className="border-b border-gray-200 bg-gray-50/80 p-5 relative">
                ...
                <div className="flex items-start gap-3 pr-6">
                  ...
                  <div>
                    <h2 className="text-base font-bold text-gray-600">{selectedQuoteForDrawer.quotationNo}</h2>
                ...
                <div className="mt-3 flex items-center justify-between pt-2 border-t border-gray-200">
                  ...
                  } ${STATUS_CONFIG[selectedQuoteForDrawer.status]?.text || "text-gray-600"}`}>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4 text-xs">
                ...
                <div className="rounded-sm border border-gray-200 bg-white p-3.5 space-y-2">
                  ...
                  <p className="font-bold text-sm text-gray-600">{selectedQuoteForDrawer.customer?.name || "Client"}</p>
                ...
                {selectedQuoteForDrawer.note && (
                  <div className="rounded-sm border border-gray-200 bg-gray-50/50 p-3.5">
                    <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wider block mb-1">Notes & Conditions</span>
                    <p className="text-gray-600">{selectedQuoteForDrawer.note}</p>
                  </div>
                )}
              </div>
              ...
              <div className="border-t border-gray-200 bg-gray-50/80 p-4 flex items-center justify-between gap-2">
                {selectedQuoteForDrawer.status === "ACCEPTED" ? (
                  <button
                    ...
                    className="w-full inline-flex items-center justify-center gap-1.5 rounded-sm bg-brand-primary py-2 text-xs font-semibold text-white shadow-sm hover:bg-brand-dark transition"
                  >
                  ...
                  <button
                    ...
                    className="w-full inline-flex items-center justify-center gap-1.5 rounded-sm border border-gray-300 bg-white py-2 text-xs font-semibold text-gray-600 hover:bg-gray-50 transition"
                  >
