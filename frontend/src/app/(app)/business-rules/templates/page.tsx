      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {TEMPLATES.map((t) => (
          <Link key={t.name} href="/business-rules/create"
            className="group rounded-sm border border-gray-100 bg-white p-5 shadow-sm transition hover:border-primary-300 hover:shadow-md">
            <div className="flex items-start justify-between">
              <span className="text-2xl">{t.icon}</span>
              <span className="rounded-sm bg-gray-100 px-2 py-0.5 text-[10px] font-bold text-gray-500">{t.trigger}</span>
            </div>
            <h3 className="mt-3 font-semibold text-gray-600 group-hover:text-primary-700">{t.name}</h3>
            <div className="mt-2 rounded-sm bg-gray-50 p-3 font-mono text-xs text-gray-600">
