...
export default function TimeSlotsManager() {
  ...
  return (
    <div className="space-y-5 animate-fadeIn">
      ...
      <div className="rounded-sm border border-slate-200 bg-white p-5 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <div className="p-3 rounded-sm bg-brand-50 border border-brand-border text-brand-dark">
            <Clock size={24} />
          </div>
          <div>
            <div className="flex items-center gap-2">
              ...
              {currentTime && (
                <span className="px-2 py-0.5 rounded-full text-[11px] font-mono font-bold bg-slate-100 text-gray-600 border border-slate-200">
                  Clock: {currentTime}
                </span>
              )}
            </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-3 px-4 py-2.5 rounded-sm border transition-all duration-200 cursor-pointer shadow-xs ${
              filterEnabled
                ? "bg-brand-50 border-brand-border text-brand-dark ring-2 ring-brand-border"
                : "bg-slate-50 border-slate-300 text-gray-600 hover:bg-slate-100"
            }`}
          ...
          >
            <div className="flex flex-col text-left">
              ...
              <span className="text-[11px] font-bold mt-0.5">
                {filterEnabled ? (
                  <span className="text-brand-dark font-bold">🟢 ON (Filtered by Shift)</span>
                ) : (
            ...
            <div
              className={`w-12 h-6 rounded-full transition-colors relative p-0.5 shrink-0 ${
                filterEnabled ? "bg-brand-primary" : "bg-slate-300"
              }`}
            >
              <div
          ...
          <Link
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-sm border border-indigo-200 bg-indigo-50 hover:bg-indigo-100 text-indigo-800 text-xs font-bold shadow-xs transition"
          >
          ...
          <CustomButton
            ...
            size="sm"
            className="bg-brand-primary hover:bg-brand-dark text-white font-bold text-xs py-2"
          >
      ...
      {activeSlot && (
        <div className="p-4 rounded-sm border border-amber-200 bg-gradient-to-r from-amber-50/90 to-orange-50/80 shadow-xs flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <span className="p-2 rounded-sm bg-amber-500 text-white shadow-xs">
              <Sun size={18} />
            </span>
            <div>
              <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                {/* Timing Badge */}
                <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-sm bg-slate-50 border border-slate-200 font-mono text-xs font-bold text-gray-600 my-2">
        ...
        {slots.length === 0 && !loading && (
          <div className="col-span-full rounded-sm border border-dashed border-slate-300 bg-slate-50/60 p-8 text-center space-y-3">
            ...
            <h3 className="font-bold text-gray-600 text-sm">No Meal Shifts Created Yet</h3>
            ...
            <div className="pt-2 flex justify-center gap-3">
              ...
              <CustomButton
                onClick={handleOpenCreateModal}
                size="sm"
                className="bg-brand-primary hover:bg-brand-dark text-white text-xs font-bold"
              >
        ...
        <form onSubmit={handleSaveSlot} className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="h-9 w-12 rounded-sm border border-slate-300 p-0.5 cursor-pointer bg-white"
                />
          ...
          <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
            ...
            <CustomButton
              ...
              size="sm"
              disabled={submitting}
              className="bg-brand-primary hover:bg-brand-dark text-white font-bold text-xs px-5"
            >
